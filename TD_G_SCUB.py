...
def GSCUB (segmentation):
    ...
    candidate_elements = all_edges[:]    
    
    all_candidate_value = []
    all_candidate_output = []
    edge_index_in_summary = []
    for iteration in all_edges:
        candidate_value = []
        candidate_edge = []
        ...
        for e in candidate_elements:
            ...
            objective_value = sum([float(temporary_index_list[i]/(size_list[i]+len(output)+1)) for i in range(M)]) 
        ...
        candidate_elements.remove(selected_edge)
        ...
        for i,o in enumerate(options):
            if i == 0:
                if (o[1][0],o[1][1]) not in all_dis_dict:
                    error, representative = GSCUB(sub_segment[1:])
            ...
            if i == 1:
                if (o[0][0],o[0][1]) not in all_dis_dict:
                    error, representative = GSCUB(sub_segment[0:2])
    ...
    if len(sub_segment) > 3:
        ...
        for j in index_sub_segment[1:-2]:
            ...
            if (start,j) not in all_dis_dict:
                error, representative = GSCUB(sub_segment[0:anchor_index+1])
                all_dis_dict[(start,j)] = [error, representative]
            if (j+1,end) not in all_dis_dict:    
                error, representative = GSCUB(sub_segment[anchor_index+1:])
        ...
        if (start,index_sub_segment[-2]) not in all_dis_dict:
            error, representative = GSCUB(sub_segment[0:len(index_sub_segment)-1])
